...
platform = {
    ...
    -- PLIC (match w/ DTS configuration)
    plic_0 = {
    ...
    -- ACLINT SWI (pass last CPU reference to avoid segfault)
    swi_0 = {
        moduletype = "riscv_aclint_swi",
        args = {"&platform.qemu_inst", "&platform.cpu_4"},  -- Reference last CPU
    ...
    mtimer_0 = {
        ...
        num_harts = 5, -- TODO: link with RISCV_NUM_CPUS variable
    };

    -- Main memory (match w/ DTS configuration)
    -- Start at _OPENSBI_LOAD_ADDR, 14G RAM 
    ram_0 = {
        moduletype = "gs_memory",
        target_socket = {address=0x80000000, size=0x380000000, priority=2048, bind = "&router.initiator_socket"},
        reset = { bind = "&reset.reset" },
    };

    -- Low memory for bootloader and peripherals
    ram_low = {
        moduletype = "gs_memory",
        target_socket = {address=0x0, size=0x80000, priority=2048, bind = "&router.initiator_socket"},
        reset = { bind = "&reset.reset" },
    };

    -- UART (match w/ DTS configuration)
    uart_0 = {
    ...
    -- PCIe controller (optional, remove if not needed)
    gpex_0 = {
...
-- Create GPUs (optional)
platform["with_gpu"] = false; -- No GPUs in system
